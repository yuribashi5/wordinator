...
import net.sf.saxon.lib.FeatureKeys; // ~.lib.Feature was not found (previous line)
 ...
 * 
 * <p>
 * You can use this directly as the main file run from the command line or as a
 * helper class to build your own command-line handler or integrated DOCX
 * generator.
 *
 */
public class MakeDocx {
	 ...
	 * command line processor if you need additional command-line options, for
	 * example, to set additional XSLT parameters.
	 * 
	 ...
	 */
	public static void handleCommandLine(Options options, String[] args, Logger log) throws Exception {
		...
		log.info(
				"XSLT template              =" + (transformPath == null ? "Not specified" : "'" + transformPath + "'"));
		...
		if (!templateFile.exists()) {
			throw new RuntimeException(
					"Template file '" + templateFile.getAbsolutePath() + "' not found. Cannot continue.");
		}
		...
		try {
			...
			throw new RuntimeException(e.getClass().getSimpleName() + " loading template DOCX file \""
		...
		if (!outDir.exists()) {
			...
			if (!outDir.mkdirs()) {
				...
				throw new RuntimeException(
						"Failed to create output directory '" + outDir.getAbsolutePath() + "'. Cannot continue");
			}
		}
		...
		if (null != transformPath) {
			...
			if (!transformFile.exists()) {
				...
				throw new RuntimeException(
						"XSLT transform file '" + transformFile.getAbsolutePath() + "' not found. Cannot continue.");
			}
	 ...
	 * @param transformFile  The file containing the XSLT transform for generating
	 *                       SWPX documents
	 * @param catalog        List of catalog files (as for Saxon -catalog option).
	 *                       Maybe null.
	 * @param xsltParameters Map of parameter names to values to be passed to the
	 *                       XSLT transform.
	 * @param log            Log to write messages to.
	 * @throws Exception Any kind of error
	 */
	public static void transformXml(File docFile, File outDir, XWPFDocument templateDoc, File transformFile,
			String catalog, Map<String, String> xsltParameters, Logger log) throws Exception {
		// Apply transform to book file to generate Simple WP XML documents

		if (transformFile == null) {
			throw new RuntimeException(
					"-x (transform) parameter not specified. If the input is a _Book.xml file, you must specify the -x parameter");
		}
		...
		// Saxon 9.9+ version:
		// processor.setConfigurationProperty(Feature.OUTPUT_URI_RESOLVER,
		// outputResolver);
		...
		if (catalog != null) {
			...
			try {
				...
				if (klass == null) {
					throw new RuntimeException(
							"-k/-catalog option specified but failed to load class " + APACHE_RESOLVER_CLASS);
				}
		...
		// Raymond...Source docSource = new StreamSource(docFile.getAbsolutePath());
	 ...
	 * @param outFile     If this is a directory, result filename is constructed
	 *                    from input filename.
		...
		try {
			...
			if (effectiveOutFile.exists()) {
				if (!effectiveOutFile.delete()) {
					log.error("Could not delete existing DOCX file \"" + effectiveOutFile.getAbsolutePath()
							+ "\". Skipping SWPX file.");
					return;
				}
			}
...
//log.info("wordinator-MakeDocx-BEFORE generator.generate()");
	...
	/**
	 * Process all *.swpx files in the input directory, putting the results in the
	 * output directory.
	 * <p>
	 * NOTE: This method is primarily for testing purposes. During production the
	 * SWPX docs are generated dynamically from the _Book.xml file.
	 * </p>
	 * 
	...
	public static Options buildOptions() {
		...
		Option input = Option.builder(OPTION_CHAR_INPUTPATH).required(true).hasArg(true)
				.desc("The path and filename of the Simple WP XML document or directory containing .swpx files.")
				.build();
		Option output = Option.builder(OPTION_CHAR_OUTPUTPATH).required(true).hasArg(true)
				.desc("The path and filename of the result DOCX file, or directory to contain generated DOCX files")
				.build();
		Option template = Option.builder(OPTION_CHAR_TEMPLATEPATH).required(true).hasArg(true)
				.desc("The path and filename of the template DOTX file.").build();
		Option transform = Option.builder(OPTION_CHAR_TRANSFORMPATH).required(false).hasArg(true)
				.desc("The path and filename of the XSLT transform for generating SWPX documents.").build();
		Option dpi = Option.builder("d").longOpt("dpi").required(false).hasArg(true).desc(
				"The dots-per-inch value to use when converting pixels to absolute measurements, e.g., \"72\" or \"96\".")
				.build();

		Option chunkLevel = Option.builder(OPTION_CHAR_CHUNKLEVEL).longOpt("chunklevel").required(false).hasArg(true)
				...
				.build();
		Option catalog = Option.builder(OPTION_CHAR_CATALOG).longOpt("catalog").required(false).hasArg(true)
				.desc("Semicolon-separated list of XML catalog file names, as for the Saxon -catalog option.").build();
